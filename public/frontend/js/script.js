...
// popup report post
	...
	$('.update-report').on('click', function () {
		$('.popup-wraper3-update').addClass('active');
		return false;
	});
	$('.popup-closed, .cancel').on('click', function () {
		$('.popup-wraper3-update').removeClass('active');
		return false;
	});	
...
jQuery(".post-comt-box textarea").on("keydown", function(event) {
	...
	$.ajax({
		...
		method:"POST",
		dataType:"json",
		data:{video_uuid:vid,user_id: user_id, comment:comment},
		...
		if(response.status == 200){
			...
			var comment_HTML = '<li><div class="comet-avatar"><img alt="" src="images/resources/comet-2.jpg"></div><div class="we-comment"><h5><a title="" href="">'+username+'</a></h5><p>'+comment+'</p><div class="inline-itms"><span>Just now</span></div></div></li>';
